...
public class falsePosition {
    ...
    public String[] rootByFalsePosition(String equation,double xl,double xu,double es,int imax, int sign)   {
        validation eq = new validation();
        if (!eq.validate(equation)){
...
//        }
        if((evaluator.evaluateFunction(equation,xl)*evaluator.evaluateFunction(equation,xu)) > 0){
            JOptionPane.showMessageDialog(null, "No bracket");
            return new String[]{"null","null","null"};
        }
