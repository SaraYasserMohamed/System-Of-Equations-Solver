...
public class expression
{
  ...
  private variable_table variables_table = new variable_table();
  ...
  public Double evaluate(variable_table values)
  {

    this.post_fix_expression = this.post_fix_with_vars;
    for(int i=0;i<values.count;i++)
    {
      this.replace(values.var_names.get(i),Double.parseDouble(values.var_values.get(i)));
    }
    Stack<String> good_stack = replace_on_stack(values);
    String parsed_post_fix = parse_expression(this.post_fix_expression);
    Stack<Double> local_stack = new Stack<Double>();

    while(!good_stack.empty())
    {
      if(isDouble(good_stack.peek()))
      {
        local_stack.push(Double.parseDouble(good_stack.pop()));
      }
      else
      {
        Double n2 = local_stack.pop();
        Double n1 = local_stack.pop();
        Double ans=0.0;
  ...
  public variable_table get_variables_table()
  {
  ...
  public void replace(String var,double val)
  {
    ...
    while(index >= 0)
    {
      ...
      {
        String str_val = Double.toString(val);
  ...
  public Stack<String> replace_on_stack(variable_table values)
  {
